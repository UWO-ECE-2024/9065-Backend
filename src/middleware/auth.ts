...
export const authMiddleware = (
    ...
    try {
        ...
        if (error instanceof TokenExpiredError) {   // Handle expired token error
            return res.status(401).json({
                error: {
                    issues: [
                        {
        ...
        } else if (error instanceof JsonWebTokenError) {    // Handle invalid token format or signature
            return res.status(401).json({
                error: {
                    issues: [
                        {
        ...
        return res.status(401).json({   // Handle any other authentication errors
            error: {
                issues: [
                    {
                        code: "unauthorized",
